...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("DOC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("dotacoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non DOC unit input in lnReqAmount, we generate the URI with DOC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DOC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
