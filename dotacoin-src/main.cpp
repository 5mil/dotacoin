...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uDOC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/660.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
{
    int64 nSubsidy = 53 * COIN;

    // Subsidy is cut in half every 620000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 620000);
...
void ThreadScriptCheck() {
    RenameThread("dotacoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1391278253, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xb4628f5c << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xb4, 0x62, 0x8f, 0x5c };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DotacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DotacoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("DotacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dotacoin-miner");
    ...
    try { loop {
        ...
        printf("Running DotacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DotacoinMiner terminated\n");
        throw;
    }
}
